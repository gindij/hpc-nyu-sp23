...
//
{
  ...
  // we were accessing unallocated memory because we allocated n integers, but
  // were trying to access n+1 locations
  for ( i = 2; i < n; i++ )
  {
    x[i] = x[i-1] + x[i-2];
    cout << "  " << i << "  " << x[i] << "\n";
  ...
  // when we use malloc to allocated, we have to use free to de-allocate
  free(x);
